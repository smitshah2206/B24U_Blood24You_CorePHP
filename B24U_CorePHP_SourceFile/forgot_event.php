<?php
	...
	if(isset($_POST['submit']))
	{
				...
				{
					$sql="SELECT user_email FROM eventlogin WHERE user_email='".$_POST['user_email']."' AND status=0";
					...
					{
						echo "<script> alert('This Email is not registered on our system...!!');window.location='forgot_event_password.php' </script>";
					}
				}
				else
				{
					...
					{
						$sql="UPDATE eventlogin SET password='$password',confirm_password='$confirm_password' WHERE user_email='".$user_email."' ";
						$result=mysqli_query($conn,$sql);
						if($result)
						{
							echo "<script> alert('Your password was successfuly changed..!!!');window.location='login_event.php' </script>";
							session_destroy();
						}
						else
						{
							echo "<script> alert('Something went wrong..!!!  Please Try again..!!!');window.location='forgot_event_password.php' </script>";
							session_destroy();
						}
					}
					else
					{
						echo '<script> alert("Password & confirm_password does not match"); 
												window.location = " forgot_event.php"; </script>';
					}
